...
3. Use `requirePlan("pro")` if the page needs a minimum plan
...
- `requirePlan("pro")` — redirects to /pricing if plan insufficient
- Session is deduplicated per-request via `React.cache()`
- API routes must always check auth themselves (don't rely on middleware)

### Plan Gating
- Server: `requirePlan("pro")` or `hasMinimumPlan(session.plan, "pro")`
- Client: `<PlanGate plan={session.plan} minimum="pro">` (pass plan from server parent)
