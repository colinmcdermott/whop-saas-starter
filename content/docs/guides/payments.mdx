...
You can also set this per-plan — e.g., monthly-only for Pro but both intervals for Enterprise.
...
export const PLAN_METADATA = {
  free: { ... },
  starter: { // ← new tier, above free, below pro
    name: "Starter",
...
After adding, connect the Whop plan IDs via the setup wizard or env vars (`NEXT_PUBLIC_WHOP_STARTER_PLAN_ID`).
...
enterprise: { ..., billingIntervals: ["monthly"] },
```

## Billing Intervals

Each paid plan supports monthly and yearly billing by default. The pricing page shows a toggle, and yearly prices show the monthly equivalent (e.g., "$24/mo billed annually"). The discount is calculated automatically from your `priceMonthly` and `priceYearly` values.
