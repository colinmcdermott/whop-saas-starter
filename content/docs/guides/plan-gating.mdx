...
export default async function ProFeaturePage() {
  ...
  return <div>Welcome, {session.name}! You have Pro access.</div>;
}
```

The plan hierarchy is determined by key order in `PLAN_METADATA` (first = lowest). With the default tiers: **enterprise > pro > free**. An enterprise user can access pro-gated pages.
...
export async function POST(request: Request) {
  ...
  if (!hasMinimumPlan(session.plan as PlanKey, "pro")) {
...
export default async function DashboardPage() {
  ...
  return (
    <div>
      ...
      {/* Show upgrade prompt for free users, feature for pro+ */}
      <PlanGate
        plan={session.plan}
        minimum="pro"
...
- **Environment variables**: `WHOP_PRO_PRODUCT_ID` and `WHOP_ENTERPRISE_PRODUCT_ID`
...
|--------|----------|----------|
| `requirePlan("pro")` | Gate entire pages (redirect) | Server components |
| `hasMinimumPlan(plan, "pro")` | Check plan level (boolean) | API routes, logic |
| `<PlanGate minimum="pro">` | Conditional UI rendering | Client components |
