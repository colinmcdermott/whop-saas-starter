...
// Only pro and enterprise users can access this page
...
const session = await getSession();
if (!session || !hasMinimumPlan(session.plan as PlanKey, "pro")) {
...
The plan hierarchy is auto-derived from key order in `PLAN_METADATA` (`lib/constants.ts`). With the default tiers: `enterprise` > `pro` > `free`. All helpers respect this hierarchy — an enterprise user can access pro-gated features. If you add or remove tiers, the hierarchy updates automatically.
